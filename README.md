...
当前版本：**2.1.0 (2026-05-05)**
- **主要更新**：加固了 LaTeX 解析逻辑，支持超长报告（16k tokens），解决了环境未闭合和公式编译错误。

查看 [CHANGELOG.md](CHANGELOG.md) 了解详细更新记录。

## 📖 使用技巧与规范

为了获得最佳的 PDF 排版效果，请遵循以下建议：

1. **复杂数学公式**：本插件已通过 2.1.0 版本的加固逻辑，能够完美处理复杂的多行公式（如 `aligned`, `cases`）。建议在提问时要求 AI 尽量一次性给出完整的公式块。
2. **长篇报告生成**：当前版本已支持生成超长篇幅（15,000+ Token）的学术报告。如果遇到内容极其庞大的情况，系统会自动优化清洗流程以确保标签闭合。
3. **公式安全性**：本插件会自动识别并转义 LaTeX 中的 `<` 与 `>` 符号。如果您的公式渲染出现异常，请检查是否在公式内部使用了非标准的 HTML 字符。
